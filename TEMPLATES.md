...
| 4 | **HN Top 10 — codziennie 08:00 (Gemini)** | `KmPY4nLRoV6JwYmz` | Schedule Trigger (daily) | 13 | Google Gemini (AI Studio), Gmail account (GCP) | 2026-05-10 (przebud. 2026-05-14) |
| 5 | **HN Top 10 — codziennie 08:00 (Groq)** | `nicN5lZNI0LSb7Jz` | Schedule Trigger (daily) | 13 | Groq account (OpenAI), Gmail account (GCP) | 2026-05-14 |
...
- **Struktura:** Schedule (daily) → HTTP (topstories) → HTTP (detale HN ×500, batch 10) → Code (filtr+ranking) → **splitInBatches(1)** → HTTP (tłumaczenie ×10) → Wait 20s → Google Gemini (streszczenie ×10) → Code (formatowanie) → Data Table (zapis) → Code (HTML email) → Gmail (wysyłka)
...
- **Streszczenie:** Google Gemini (`text:message`, model domyślny) generuje 5-6 zdaniowe polskie podsumowanie każdego artykułu na podstawie tytułu i URL
...
- **Wait 20s między Gemini callami:** wymuszone limitem 5 req/min darmowego tieru Gemini
...
- **Data dodania:** 2026-05-10 (przebud. 2026-05-14 — usunięcie urlContext, maxTokens 1024→4096, prompt 2-3→5-6 zdań)

### 5. HN Top 10 — codziennie 08:00 (Groq)
- **Źródło:** Zastępuje #5 DeepSeek, darmowy model przez Groq zamiast płatnego DeepSeek
- **Struktura:** Schedule (daily) → HTTP (topstories) → HTTP (detale HN ×500, batch 10) → Code (filtr+ranking) → **splitInBatches(1)** → HTTP (tłumaczenie ×10) → **Code (merge data)** → **AI Agent \| Groq / Llama 3.3 70B (streszczenie ×10)** → Code (formatowanie) → Data Table (zapis) → Code (HTML email) → Gmail (wysyłka)
- **Trigger:** Codziennie o 08:00
- **Filtrowane frazy:** opencode, cloud code, openrouter, openai, codex, antigravity, warpdotdev, gemini, stape_io, n8n
- **Wyszukiwanie:** Top 500 HN → fetch szczegółów wszystkich 500 (batch 10) → filtr po tytułach → sort po score → top **10**
- **Tłumaczenie:** MyMemory API (EN→PL), z fallbackiem do oryginalnego tytułu
- **Streszczenie:** Groq / Llama 3.3 70B (`@n8n/n8n-nodes-langchain.lmChatGroq`) przez AI Agent (`@n8n/n8n-nodes-langchain.agent`) generuje 5-6 zdaniowe polskie podsumowanie każdego artykułu
...
- **Kredencjały:** Groq account przez `groqApi`, Gmail account (GCP) przez `gmailOAuth2`
...
| Data | Opis |
|---|---|---|
| 2026-05-14 | Przebudowano #4 (Gemini): usunięcie `urlContext`, maxTokens 1024→4096, prompt 2-3→5-6 zdań. Dodano #5 (Groq): nowy wariant z darmowym Groq/Llama 3.3 70B. Usunięto #5 (DeepSeek) — wymagał płatnego konta. |
